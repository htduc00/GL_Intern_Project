...
TowerInfo::TowerInfo(std::shared_ptr<Model> model, std::shared_ptr<Shader> shader, std::shared_ptr<Texture> texture)
	: Sprite2D(model, shader, texture), m_isActive(false), m_listButton(NULL),m_listText(NULL)
{
}
...
void TowerInfo::SetActive(bool isActive)
{
	m_isActive = isActive;
}

void TowerInfo::SetListButton(std::list<std::shared_ptr<GameButton>> listButton)
{
	m_listButton = listButton;
}

void TowerInfo::SetListText(std::list<std::shared_ptr<Text>> listText)
{
	m_listText = listText;
}
